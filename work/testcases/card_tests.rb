...
class TestCard < Test::Unit::TestCase
  ...
  def test_fail_contract
    ...
    assert_raise ContractViolation do
      aircraft = Aircraft.new(Image.new("test"), Axis.new, Axis.new, Bomber.new, 5)
    end
    
    assert_raise ContractViolation do
      aircraft = Aircraft.new(Image.new("test"), "Name", Bomber.new, Bomber.new, 5)
    end
    
    assert_raise ContractViolation do
      aircraft = Aircraft.new(Image.new("test"), "Name", Axis.new, Axis.new, 5)
    end
    
    assert_raise ContractViolation do
      aircraft = Aircraft.new(Image.new("test"), "Name", Axis.new, Bomber.new, "somethingelse")
    end
  ...
  def test_xml_to_card_aircraft
    doc = Document.new
    doc.add_element "AIRCRAFT", {"NAME"=>"Baku Geki KI-99", "TAG"=>"1"}
  ...
  def test_card_to_xml
    ex_air = Document.new
    ex_air.add_element "AIRCRAFT", {"NAME"=>"Baku Geki KI-99", "TAG"=>"1"}
   
    ex_keep = Document.new
    ex_keep.add_element "KEEPEM", {"TAG"=>"1"}
   
    ex_vic = Document.new
    ex_vic.add_element "VICTORY"
   
    assert_equal(ex_air.to_s, @air1.card_to_xml.to_s)
    assert_equal(ex_keep.to_s, @keep1.card_to_xml.to_s)
    assert_equal(ex_vic.to_s, @vic1.card_to_xml.to_s)
  end
end
