...
class Hand
  ...
  # empty? : Hand -> Boolean
  #
  # Are there any cards in this hand?
  def empty?
    self.size == 0
  end
  ...
  def minus(list)
    ...
    list.each{|card|
      new_list.delete_if{|x| x == card}
    }
    Hand.create(new_list)
  end
  ...
  # to_s : Hand -> String
  #
  # Make a readable list of cards from a Hand
  def to_s
    string = ""
    @list_of_cards.each{|card|
      string = "#{string}#{card.to_s}\n"
    }
    string
  end
  ...
  # hand_to_xml : Hand -> Document
  #
  # Converts a Hand to XML Document.
  def hand_to_xml
