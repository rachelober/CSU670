...
class Player
    ...
    pre(:player_take_turn, "Input must be an instance of Turn and there needs to be at least one card on the Deck or one card on the Stack") {|turn| (turn.instance_of?(Turn)||turn.instance_of?(ProxyTurn)) && (turn.turn_stack_inspect.size > 0 || turn.turn_card_on_deck?)}
    ...
    def play_hand(list_of_cards, list_of_allies, list_of_axis)
        ...
        discards = self.make_max_discards
    ...
    # make_max_discards : Player -> ListOf-Squadron
    #
    ...
    def make_max_discards
        ...
        # are complete.
        list_of_discards = all_squads.find_all {|squadron|
