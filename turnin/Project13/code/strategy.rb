      ...
      if wildcards.size > 0
        if squad.squadron_alliance.instance_of?(Allies) && list_of_axis.size > 0
          if wildcards.size >= 2 && squad.list_of_cards.size == 1
            ...
            squad.push!(wildcards.shift)
            attacks << Attack.new(squad, list_of_axis.shift)
          elsif squad.list_of_cards.size == 2
...
# This Strategy takes too long to make decisions. Therefore it should cause
# a timeout error.
...
class InspectorStrategy < Strategy
  ...
  def choose_cards(turn, player)
    ...
    if stack.index(possible.first).nil?
      index = 1
    else
      index = stack.index(possible.first) + 1
    end
