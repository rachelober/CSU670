...
class Squadron
  ...
  # be swapped, returns nil.
  def squadron_swap(newcard)
    return_card = @list_of_cards.detect{|card| card.wildcard?}
    if !return_card.nil?
      @list_of_cards.delete_if{|card| card == return_card}
      @list_of_cards << newcard
      @list_of_cards.sort!
    end
    return_card
  end
  ...
  def to_s
    string = "Squadron: #{squadron_name} #{how_many_aircraft?} / 3"
  end
