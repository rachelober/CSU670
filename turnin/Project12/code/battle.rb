...
class Battle  
  ...
  attr_reader :stack, :table, :playerstates, :banned
    ...
    until (battle_over?)
      ...
      thread = Thread.new do
        begin
          ...
          kick_player(player, playerstate, "BROKE TIMING")
          Thread.kill(self)
        rescue ContractViolation
          kick_player(player, playerstate, "BROKE CONTRACT")
          Thread.kill(self)
        rescue Cheating
          kick_player(player, playerstate, "CHEATED")
          Thread.kill(self)
        end
  ...
  def deal_hands  
    @playerstates.each{|playerstate|
      7.times{
        ...
        @deck = @deck.pop}
      @players.each{|player|
        if player.player_name == playerstate.name
          player.player_first_hand(playerstate.hand.hand_to_list)
        end}}
  end
    ...
    turn_attacks.each{|attack|
      @table.each{|squadstate|
        if attack.bomber.equal?(squadstate.squadron)
          ...
          previous_owner.inform_shotdown([squadstate.squadron])
  ...
  #
  # Updates the PlayerState's Hand with what happened during the Turn.
