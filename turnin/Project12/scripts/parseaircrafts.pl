...
if (@ARGV != 1){
    die "Usage: parseaircrafts.pl <filename>\n\n";
}
...
foreach my $aircraft (@{$ref->{aircraft}}){
    ...
    if(($nation eq "Japan") or ($nation eq "Germany") or ($nation eq "Italy")) {
        $aircraft->{nation} = "axis";
    }
    
    # For the Allied nations, read the contents of the nation
    # attribute and change it appropriately
    elsif(($nation eq "US AF") or ($nation eq "UK") or ($nation eq "US Navy") or ($nation eq "Soviet Union")){
        $aircraft->{nation} = "allies";
    }
    
    # Check to see if the image attribute is defined, 
    # and if not, add it with the "question.gif" value
    if(!(defined($image))) {$aircraft->{image} = "question.gif"}
    else{$aircraft->{image} = $name . ".gif"}
